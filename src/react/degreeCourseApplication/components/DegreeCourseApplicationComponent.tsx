...
export function DegreeCourseApplicationComponent({ bewerbung, degreeCourseApplicationAktualisieren }: DegreeCourseApplicationComponentProps) {
    ...
    const [abfrageFehlerhaft, setAbfrageFehlerhaft] = useState(false);
    ...
    let studiengangGeloescht = <div></div>;
    if (studiengang === undefined && abfrageFehlerhaft) { //wenn Studiengang geloescht wird Berwerbung nicht automatisch auch geloescht, aber Meldung wird angezeigt, wird erst gemacht wenn Abfrage fertig ist
        studiengangGeloescht =  <div style={{color: "red"}}>Der Studiengang zu dieser Bewerbung existiert nicht mehr.</div>;
    }

    if (accessToken !== null) {
        return <div><Card id={"DegreeCourseApplicationItem" + bewerbung.id} style={{ minWidth: "200px" }}>
            <Card.Header><b>{bewerbung.applicantUserID}: {studiengang?.universityShortName}{bewerbung.targetPeriodYear} {bewerbung.targetPeriodShortName}</b> {studiengangGeloescht}</Card.Header>
            <ListGroup>
